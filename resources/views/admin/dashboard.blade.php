...
<x-layout>
    <x-card>
        <div>
            <h3 class="text-3xl font-bold mb-4 text-center">Explore All Projects</h3>
            <p class="text-md mb-4 text-center">Projects are prioritized by unassigned locations.</p>
        </div>
            ...
            @unless(count($projects) == 0)
                @foreach($projects as $project)
                    <x-project-card :project="$project"
                        :bg="($project->isAssigned()) ? 'border-blue-500' : 'border-red-500' " />
                @endforeach
            @else
